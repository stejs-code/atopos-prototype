...
export class ImportRegistry {
      ...
      for (const spec of specs) {
        if (spec.type === 'ImportDefaultSpecifier') {
          this.localToInfo.set(spec.local.name, {
            importPath, symbol: 'default', kind: 'default', importDecl: node, spec
          })
        } else if (spec.type === 'ImportSpecifier') {
          const imported = spec.imported.type === 'Identifier' ? spec.imported.name : spec.imported.value
          this.localToInfo.set(spec.local.name, {
            importPath, symbol: imported, kind: 'named', importDecl: node, spec
          })
        } else if (spec.type === 'ImportNamespaceSpecifier') {
          this.localToInfo.set(spec.local.name, {
            importPath, symbol: '*', kind: 'namespace', importDecl: node, spec
          })
  ...
  getInfo(localName: string) { return this.localToInfo.get(localName) }
  getNamespacePath(nsLocal: string) { return this.nsToPath.get(nsLocal) }
  ...
  *keptSpecifiersByDecl() { yield* this.declToKeptSpecs.entries() }
  getAllSpecsForDecl(decl: any) { return this.declToAllSpecs.get(decl) || [] }
  getSource() { return this.code }
}
