...
import type { ImportRegistry } from './import_registry'
      ...
      if (nsSpec) {
        ...
        if (namedSpecs.length) {
          const inner = namedSpecs
            .map((sp: any) => {
              const importedName =
                sp.imported.type === 'Identifier' ? sp.imported.name : sp.imported.value
              return importedName === sp.local.name
                ? sp.local.name
                : `${importedName} as ${sp.local.name}`
            })
            .join(', ')
          parts.push(`{ ${inner} }`)
        }
