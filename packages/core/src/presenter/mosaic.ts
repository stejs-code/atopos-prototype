...
import { PresenterClass } from './types.js'
import z from 'zod/v4'
import { MethodMetadata } from '../manifest/types.js'
...
export class Mosaic {
  ...
  constructor(
    presenterID: string | PresenterID, // $ for null
    actionID: string | ActionID, // $ for null
  ...
  static make<T extends PresenterClass, M extends ActionMethods<T>>(mosaic: MosaicInput<T, M>) {
    if (typeof mosaic === 'string') return Mosaic.makeFromString(mosaic)

    if (mosaic instanceof Mosaic) return new Mosaic(mosaic.presenterID, mosaic.actionID)
  ...
  static makeFromString(string: string) {
    ...
    let [presenter, method] = main.split(':')
  ...
  static makeFromGetter(mosaic: [PresenterClass | string, (i: any) => any, ...args: any[]]) {
...
type ActionMethods<T extends PresenterClass> = InstanceType<T>[ActionKeys<T>]

type ActionKeys<T extends PresenterClass> = {
  [K in keyof InstanceType<T> & string]: K extends `action${string}` ? K : never
}[keyof InstanceType<T> & string]

export type MosaicInput<T extends PresenterClass, M extends ActionMethods<T>> =
  | Mosaic
  | [
      T,
      (p: InstanceType<T>) => M,
